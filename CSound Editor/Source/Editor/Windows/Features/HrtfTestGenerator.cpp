...
HrtfTestGenerator::HrtfTestGenerator()
{
	generatedPanel = nullptr;
	InitUI();
}

void HrtfTestGenerator::InitUI()
{
...

	{
		...
		QObject::connect(button, &QPushButton::clicked, this, &HrtfTestGenerator::Clear);
	}
...
void HrtfTestGenerator::SetupGenerator(vector<float> azimuthSamples, vector<float> elevationSamples)
{
...
unsigned int HrtfTestGenerator::GetNumberOfSamples() const
{
	return azimuthTestValues.size();
}

const vector<float>& HrtfTestGenerator::GetAzimuthValues() const
{
	return azimuthTestValues;
}

const vector<float>& HrtfTestGenerator::GetElevationValues() const
{
	return elevationTestValues;
}

void HrtfTestGenerator::ButtonClick(unsigned int index)
{
